...
public class Driver1 {
    ...
    public static void main(String[] _args) {
        ...
        Student temp_s = null;
        Course course = null;
        CourseOpening courseopening = null;
        ...
        while (true) {
            ...
            switch (command) {
                ...
                case "course-open":
                courseopening = checkDuplicateCorCourseopening(data[0], data[1],data[2]);
                if (courseopening != null) {
                    break;
                } else {
                    insial = data[3].split(koma);
                    ...
                    data[3] = kata;
                    Course x = checkDuplicateCorCourse (data[0]);
                    CourseOpening newCourseOpening = arrayToCourseOpening(x,data[1],data[2],data[3]);
                    addCourseOpening(newCourseOpening);
                    kata = "";
                    break;
                }
                ...
                case "enrollment-add":
                    ...
                    if (enrollment != null) {
                        ...
                        if (temp_g == null) {
                            courseopening = checkDuplicateCorCourseopening(data[0],data[3],data[4]);
                            DaftarNilai newdaftarNilai = new DaftarNilai(data[0], data[1], courseopening.getCredit());
                ...
                case "enrollment-remedial":
                    ...
                    break;
                case "course-history" :
                // courseopening = checkDuplicateCorCourseopening(data[0]);
                for (CourseOpening x : courseOpenings){
                    if(x.getCode().equals(data[0])){
                        System.out.println(x);
                    }
                    for(Enrollment y : enrollments){
                        if(y.getcode().equals(data[0]) && y.getyear().equals(x.getYear()) && y.getsemester().equals(x.getSemester())){
    ...
    public static CourseOpening checkDuplicateCorCourseopening( String CODE, String year, String Semester) {
        for (CourseOpening x : courseOpenings) {
            if (x.getCode().equals(CODE) && x.getSemester().equals(Semester) && x.getYear().equals(year)) {
                return x;
            }
        }
        return null;
    ...
    public static CourseOpening checkDuplicateCorCourseopenings(String CODECourse) {
        for (CourseOpening x : courseOpenings) {
            if (x.getCode().equals(CODECourse )) {
                return x;
            }
        }
        return null;
    ...
    private static void addCourseOpening(CourseOpening courseopening) {
        courseOpenings = Arrays.copyOf(courseOpenings, courses.length + 1);
    ...
    private static CourseOpening arrayToCourseOpening(Course course,String year,String grade,String inslist ) {
        CourseOpening courseOpening = new CourseOpening(course,year,grade,inslist);
